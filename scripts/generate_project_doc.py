...
def generate_module_documentation(
    root_dir: Path, exclude_dirs: List[str] = None
) -> str:
    """
...
def generate_file_content_documentation(
    ...
    for py_file in list(root_dir.glob("*.py")) + list(
        (root_dir / "src").glob("*.py") if (root_dir / "src").exists() else []
    ):
    ...
    for pattern in include_patterns:
        ...
        for file_path in root_dir.glob("**/*"):
            ...
            if any(
                pattern.search(file_path.name) for pattern in include_patterns
            ):
    ...
    if file_count >= max_file_count:
        content_docs.append(
            f"⚠️ Only showing {max_file_count} files to avoid excessive output size.\n"
        )
...
def calculate_project_stats(
    ...
    for file_path in files_to_process[:1000]:
        ...
        stats["largest_files"].append(
            (str(file_path.relative_to(root_dir)), file_size)
        )
        ...
        stats["newest_files"].append(
            (str(file_path.relative_to(root_dir)), modification_time)
        )
        stats["newest_files"] = sorted(
            stats["newest_files"], key=lambda x: x[1], reverse=True
        )[:5]
...
def generate_project_documentation(
    ...
    if project_name is None:
        ...
        if project_name is None:
            project_name = (
                root_dir.name.replace("_", " ").replace("-", " ").title()
            )
    ...
    try:
        ...
        doc.append(
            f"- **Total Lines of Code:** {stats['total_lines_of_code']:,}"
        )
    ...
    doc.append(
        generate_file_content_documentation(
            root_dir, max_file_count=max_file_count
        )
    )
    ...
    if generate_html:
        ...
        try:
            ...
            has_pygments = False
            print(
                "pygments not installed. Code syntax highlighting in HTML will be basic."
            )
